...
impl Protobuf {
    ...
    fn format_schema(schema: &Schema) -> Result<String, &'static str> {
        match schema {
            ...
            Schema::ObjectSchema { name, fields, unknown: _ } => {
                match name {
                    Some(name) => { Protobuf::format_message(name, fields) }
    ...
    fn format_message(name: &String, fields: &Vec<(String, FieldRestriction)>) -> Result<String, &'static str> {
        let mut field_count = 0;
        fields.iter()
            .fold(Ok(format!("message {} {{", name)), |acc, (k, v)| {
                acc.and_then(|mut output| {
                    match &v.base {
        ...
        match schema {
            Schema::ObjectSchema { name, fields, unknown } => {
                if *unknown {
                    ...
                    Protobuf::format_message(&checked_name, &fields)
                        .map(|rewrite_message| {
                            let mut output = "\n\t".to_string();
    ...
    fn format_one_of(schemas: &Vec<Schema>, start_index: &usize) -> Result<(String, String), &'static str> {
        let mut neededMessages = "".to_string();
        let mut fields = "{".to_string();
        let mut index = start_index.clone();
        let mut neededMessages_index = 0;
        schemas.iter().fold(Ok((neededMessages, fields)), |acc, schema| {
            acc.and_then(|(mut neededMessages, mut fields)| {
                let res = match schema {
                    Schema::ArraySchema { item } => {
                        Protobuf::format_schema(item.deref())
                            .map(|converted| {
                                neededMessages_index = neededMessages_index + 1;
                                neededMessages.push_str(format!("\n\tmessage Array{} {{repeated {} values = 1;}}", neededMessages_index, converted).as_str());
                                fields.push_str(format!("\n\t\tArray{n} oneOf{m} = {m};", n = neededMessages_index, m = index).as_str());
                                (neededMessages, fields)
                            })
                    }
                    sch => {
                        Protobuf::format_schema(sch)
            ...
            .map(|(mut neededMessages, mut fields)| {
                if neededMessages.len()> 0 {
                    neededMessages.push_str("\n\t")
                }
                fields.push_str("\n\t}");
                (neededMessages, fields)
            })
