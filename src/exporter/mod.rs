...
impl TypescriptGRPC {
    ...
    fn format_interface_name(name: &String) -> Result<String, &'static str> {
    ...
    fn format_interface(name: &String, fields: &Vec<(String, FieldRestriction)>) -> Result<String, &'static str> {
        TypescriptGRPC::format_interface_name(name)
            .and_then(|name| {
                TypescriptGRPC::format_object(fields).map(|converted| {
    ...
    fn format_object(fields: &Vec<(String, FieldRestriction)>) -> Result<String, &'static str> {
        fields.iter()
            .fold(Ok(" {".to_owned()), |acc, (k, v)| {
                acc.and_then(|mut output| {
                    TypescriptGRPC::format_schema(&v.base).map(|converted| {
                        output.push_str("\n\t");
    ...
    fn format_one_of(schemas: &Vec<Schema>) -> Result<String, &'static str> {
        schemas.iter()
            .fold(Ok(Vec::new()), |acc, sch| {
                acc
                    .and_then(|mut output| {
                        TypescriptGRPC::format_schema(sch).map(|converted| {
...
mod tests {
    ...
    fn export_empty_schema() {
        ...
        assert_eq!(TypescriptGRPC::export_types(&ir), Ok(expected));
    }
    ...
    fn export_object_schema_with_primitive() {
        ...
        ir.add_schema(Schema::ObjectSchema { name: Some("IInterface".to_string()), fields, unknown: true });
        assert_eq!(TypescriptGRPC::export_types(&ir), Ok(expected));
    }
